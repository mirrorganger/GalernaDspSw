...
- `firmware_core/` (renamed from CubeMX's `Core/` to avoid colliding with `galerna/core`,
  own `CMakeLists.txt`, also now home to `startup_stm32f405xx.s` and
  `STM32F405XX_FLASH.ld`) and `drivers/` (renamed from `Drivers/`, own `CMakeLists.txt`)
  were originally scaffolded by STM32CubeMX but are no longer regenerated — the board's
  pinout is finalized, and `Galerna.ioc` has been retired to
